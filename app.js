...
class App {
    ...
    renderStationResults() {
        ...
        // Detailed metrics (hidden by default, shown on "View Details")
        let detailsHTML = `
            <div class="details-section" id="detailsSection" style="display:none;">
                <div class="results-metrics">
                    <div class="metric-card"><span class="metric-title">Elapsed Time</span><span class="metric-val">${results.elapsedTime}s</span></div>
                    ...
                    <div class="metric-card"><span class="metric-title">Slowest Rep</span><span class="metric-val">${results.slowestRep}s</span></div>
                    <div class="metric-card"><span class="metric-title">Consistency</span><span class="metric-val">${results.consistency}</span></div>
                    ${!isSTS && results.averageReach ? `<div class="metric-card"><span class="metric-title">Avg Reach</span><span class="metric-val">${results.averageReach}× torso</span></div>` : ''}
                </div>
            </div>`;
        ...
        this.container.innerHTML = `
            <div class="screen results-screen">
                ...
                <button class="btn btn-link" id="btnDetails">▼ View Details</button>
                ${detailsHTML}
    ...
    renderFinalResults() {
        ...
        const stsR = this.sitToStandResults;
        const rowR = this.seatedRowResults;
        ...
        this.container.innerHTML = `
            <div class="screen final-results-screen">
                ...
                <div class="score-breakdown">
                    <div class="breakdown-row"><span>Sit-to-Stand</span><span>${stsR?.reps || 0}</span></div>
                    <div class="breakdown-row"><span>Seated Row</span><span>${rowR?.reps || 0}</span></div>
                ...
                <button class="btn btn-link" id="btnFinalDetails">▼ View Detailed Metrics</button>
                <div class="details-section" id="finalDetailsSection" style="display:none;">
                    <div class="final-station-card">
                        <h4>Station 1: Sit-to-Stand</h4>
                        <p>Reps: <strong>${stsR?.reps || 0}</strong></p>
                        <p>Avg Rep Time: ${stsR?.averageRepTime || '-'}s</p>
                        <p>Fastest Rep: ${stsR?.fastestRep || '-'}s</p>
                        <p>Slowest Rep: ${stsR?.slowestRep || '-'}s</p>
                        <p>Consistency: ${stsR?.consistency || '-'}</p>
                    </div>
                    <div class="final-station-card">
                        <h4>Station 2: Seated Row</h4>
                        <p>Reps: <strong>${rowR?.reps || 0}</strong></p>
                        <p>Avg Rep Time: ${rowR?.averageRepTime || '-'}s</p>
                        <p>Fastest Rep: ${rowR?.fastestRep || '-'}s</p>
                        <p>Slowest Rep: ${rowR?.slowestRep || '-'}s</p>
                        <p>Consistency: ${rowR?.consistency || '-'}</p>
                        <p>Avg Reach: ${rowR?.averageReach || '-'}× torso length</p>
                    </div>
                </div>
        ...
        document.getElementById('btnFinalDetails').onclick = () => {
            const section = document.getElementById('finalDetailsSection');
            const btn = document.getElementById('btnFinalDetails');
            if (section.style.display === 'none') { section.style.display = 'block'; btn.textContent = '▲ Hide Details'; }
            else { section.style.display = 'none'; btn.textContent = '▼ View Detailed Metrics'; }
        };
        document.getElementById('btnSave').onclick = () => {
            this.results.saveChallengeResult(sessionResult);
